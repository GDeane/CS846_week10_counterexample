...
This is a CS846 course assignment repository for Week 10 (CodeReview/PR). The goal is to document counterexample
...
- `feedback.md` — The primary deliverable. A structured document with three sections per problem:
    ...
    2. **Guidelines that didn't work** — a specific guideline (e.g., "Use a fault model") that GitHub Copilot failed to
...
Problems follow a placeholder convention: the `replace_with_problem_X` files in each subdirectory are placeholders to be
replaced with the actual problem code and test files.
...
- The document uses "GitHub Copilot (GPT-4.1)" as the AI model label for outputs (or update with the actual model
  used)
- Note the GenAI contribution model used to make each problem in the starter code section
